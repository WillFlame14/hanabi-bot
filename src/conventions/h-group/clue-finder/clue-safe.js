...
export function clue_safe(state, clue) {
	...
	let finessed_card = hypo_state.hands[next_unoccupied].find(c => c.finessed && playableAway(hypo_state, c) === 0);
	...
	while (finessed_card && next_unoccupied !== state.ourPlayerIndex) {
		...
		finessed_card = hypo_state.hands[next_unoccupied].find(c => c.finessed && playableAway(hypo_state, c) === 0);
	}
	...
	if (next_unoccupied !== state.ourPlayerIndex) {
		...
		let finessed_card2 = hypo_state.hands[next_unoccupied2].find(c => c.finessed && playableAway(hypo_state, c) === 0);
		...
		while (finessed_card2 && next_unoccupied2 !== state.ourPlayerIndex) {
			...
			finessed_card2 = hypo_state.hands[next_unoccupied2].find(c => c.finessed && playableAway(hypo_state, c) === 0);
		}
